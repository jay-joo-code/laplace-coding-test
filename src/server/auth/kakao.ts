...
5. 카카오 로그인 > 활성화 설정 ON, redirect URI `${process.env.DEV_SERVER_DOMAIN}/api/public/auth/kakao/callback` 등록
6. 동의항목 > 프로필 정보 설정 > 필수 (필요시 수집 체크)
*/

passport.use(new KakaoStrategy({
  clientID: process.env.ID_KAKAO,
  callbackURL: `${process.env.DEV_SERVER_DOMAIN}/api/public/auth/kakao/callback`,
},
